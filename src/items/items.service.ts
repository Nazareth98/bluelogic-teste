...
export class ItemsService {
  ...
  create(itemToCreate: CreateItemDto) {
    if (!itemToCreate.id)
      throw new BadRequestException('O ID precisa ser um inteiro válido.');
    if (!itemToCreate.name)
      throw new BadRequestException('O Nome precisa ser um texto válido.');
    if (!itemToCreate.description)
      throw new BadRequestException('A Descrição precisa ser um texto válido.');
    const itemWithSameId = global.items.find(
      (item) => item.id === itemToCreate.id,
    );
    if (itemWithSameId)
      throw new BadRequestException('Já existe um Item com o mesmo ID.');
    global.items.push(itemToCreate);
  }

  update(id: number, itemToUpadate: UpdateItemDto) {
    if (!itemToUpadate.name)
      throw new BadRequestException('O Nome precisa ser um texto válido.');
    if (!itemToUpadate.description)
      throw new BadRequestException('A Descrição precisa ser um texto válido.');
